...
function ProductCard(props: ProductCardProps) {
  const formatPrice = (price: number): string => {
    const integer = Math.floor(price).toLocaleString().replaceAll(",", ".");
    const deciminal = (price % 1).toFixed(2).slice(2, 4)

    return `${integer},${deciminal} TL`;
  }
              ...
              props.offers.slice(0, 2).map((topoffer: any, key: number) =>
                <OfferButton key={key} href={topoffer.merchant.url} target="_blank">
                  <OfferInfo>
                    ...
                    <OfferPrice>{formatPrice(topoffer.price)}</OfferPrice>
                  </OfferInfo>
                  <OfferForward>
